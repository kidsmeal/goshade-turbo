...
func _check_split_persistence(plugin: EditorPlugin, panel: GSTMainPanel) -> void:
	...
	await _drag_splitter(plugin, main_split, -(main_before["editing_rect"].size.x - main_before["tab_breakpoint"] + 24.0))
	...
	_check("split_persistence", ratios_bounded and inner_ok and main_ok, "inner=%.3f->%.3f->%.3f main=%.3f->%.3f->%.3f" % [before["inner_ratio"], inner_changed["inner_ratio"], inner_restored["inner_ratio"], main_before["main_ratio"], main_changed["main_ratio"], main_restored["main_ratio"]])
	DisplayServer.window_set_size(Vector2i(1366, 768))
	panel.restore_layout_metadata_snapshot(metadata_before)
	for i: int in range(5):
		await plugin.get_tree().process_frame
...
func _check_long_stack(plugin: EditorPlugin, panel: GSTMainPanel) -> void:
	...
	_check("stable_scroll", list.get_item_count() == 20 and selected_ok and reorder_result["ok"] and scrollable and after_reorder_anchor == before_anchor and after_refresh_anchor == before_anchor and fixed_rows, "items=%d selected=%s reorder=%s anchor=%s/%s/%s fixed_rows=%s" % [list.get_item_count(), selected_ok, reorder_result["ok"], before_anchor, after_reorder_anchor, after_refresh_anchor, fixed_rows])
...
func _check_native_preview_edit(plugin: EditorPlugin, panel: GSTMainPanel) -> void:
	...
	_check("native_preview_edit", property != null and before != null and after != null and not _images_equal(before, after), "editor_property=%s image_before=%s image_after=%s" % [property != null, before.get_size() if before != null else Vector2i.ZERO, after.get_size() if after != null else Vector2i.ZERO])
