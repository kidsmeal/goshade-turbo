...
func run(plugin: EditorPlugin) -> void:
	...
	_check("param_native_hint", int(gain_property_info.get("type", -1)) == TYPE_FLOAT and int(gain_property_info.get("hint", -1)) == PROPERTY_HINT_RANGE and String(gain_property_info.get("hint_string", "")) == GSTLayer.range_hint_string("float", float(gain_schema.get("min")), float(gain_schema.get("max"))), "type=%s hint=%s hint_string='%s'" % [gain_property_info.get("type"), gain_property_info.get("hint"), gain_property_info.get("hint_string")])
