...
Export writes a self-contained `.gdshader`: a `// Generated by GoShade
Turbo.` line, a copyright line for each included function ported from a
third-party source (with the MIT permission notice once), one
`// stack: <json>` line carrying the whole stack, then the shader body.
The export claims no license for your stack. Opening that file in the
panel rebuilds the stack from the header. No `.tres` needed.
