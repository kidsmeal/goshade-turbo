...
- Filters (`samples_source: true`): the input must be a source layer; codegen emits `texture(TEXTURE, uv + offset)` or the screen equivalent inside the filter function. Any other input is refused at slot assignment.
