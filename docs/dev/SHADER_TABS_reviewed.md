...
Implementation defaults below are implementation decisions within the approved tab feature. User decisions are identified explicitly.
...
8. Saving (implementation): Save/Save As/Export act on the initiating document. Pending dialog state carries its stable ID; switching cannot redirect a response. A failed save leaves dirty state and the document open. Save As onto a canonical path already owned by another open document is refused with a specific message. Export does not clear the stack's unsaved marker. Beat: writing one tab into another's path.
9. Closing (implementation): unmodified saved documents close immediately. Dirty documents offer Save, Discard, Cancel; Save on an untitled document runs Save As and closes only after success. Closing selects the adjacent remaining tab. Closing the last tab restores the existing recipe/Open/Create Empty Stack entry surface. Close/navigation are UI operations outside stack undo. Beat: data loss or undo reopening discarded tabs.
10. Shutdown (implementation, amended 2026-09-15 after phase 7/8 evidence): `_get_unsaved_status("")` lists the dirty shader documents that still need shutdown attention (dirty with no current recovery record, `GSTDocument.needs_shutdown_attention()`). It cannot list every dirty document: Godot re-checks `get_unsaved_status()` after `save_external_data()` (`editor_node.cpp`, `p_confirmed = false`) and reopens the confirmation while anything is still listed, and a recovered document stays dirty by design, so listing it would trap the quit. If the user chooses Save and Quit, `_save_external_data()` synchronously saves dirty documents that have paths and writes every untitled or failed-path document as a self-contained recovery stack plus validated metadata under `EditorInterface.get_editor_paths().get_project_settings_dir()/goshade_turbo/recovery`. On the next project open, recovery documents reopen as dirty tabs before the normal entry surface; each record remains until that document is saved or discarded, and invalid or unreadable records are reported by path and never deleted or rewritten. A recovery record never overwrites a user stack or exported shader. If both the requested save and the recovery write fail, report both exact paths through an editor error; `_save_external_data()` is void, so the plugin cannot block the quit itself, and the engine's own re-check is what gates it. Beat: close protection working only on the tab close button.
11. Session scope (implementation): keep open documents through main-screen and scene switches for the current editor session. General saved-tab restoration and crash recovery are deferred. Shutdown recovery in decision `10` covers only dirty documents presented to Godot's confirmed quit flow. Preserve existing shared layout persistence. Beat: expanding the initial feature into a full session-restore system.
12. Compatibility (implementation): ordinary edits, slot conversions, stable layer IDs, native Color center picker, recipe Randomize, codegen/export, and one self-contained shader retain their existing contracts. New/Open/Recipe become document-open navigation and supersede the old replacement-undo behavior for these entry points. Explicit internal replacement may remain for fixtures only when it does not escape document ownership.

```text
Godot scenes: [level.tscn x] [+]
GoShade:      [Pearl x] [Opal* x] [Untitled 1 x] +      (native TabBar, x inside the tab, flat Add icon after the newest tab; 2026-09-15)
              File  Save  Recipes  Randomize  Export   (File drawn with the Button style, Export in the left group; 2026-09-15)
              -----------------------------------------
