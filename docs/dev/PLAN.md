...
Source design: `docs/dev/DESIGN.md`
...
- **B1 (resolved):** `docs/` un-ignored in `.gitignore`. `docs/dev/DESIGN.md`, this plan, `docs/dev/SPIKE_NOTES.md`, and screenshots are tracked. Owner: user.
...
**Blockers:** none. B9 (editor spike items a-c) closed, all four spike items PASS on 4.6.2, see `docs/dev/SPIKE_NOTES.md`.
...
- `docs/dev/DESIGN.md` (modify: Codegen rules filter line per B10)
...
- `docs/dev/EDITOR_SMOKE.md` (create, per-phase record of editor smoke results, method, and engine version; phases 5-8 append)
...
- `docs/dev/DESIGN.md` (modify: decision 12 accepts a field output color layer)
- `**/*.gd.uid`, `**/*.tscn` sidecars and scenes generated or created alongside the scripts above
- `docs/dev/NOW.md`, `docs/dev/PLAN.md` (orchestrator state, edited by the pipeline scripts and the orchestrator)
...
**Verification:** editor smoke script (`godot --editor --path .` with `GST_EDITOR_SMOKE=4`, results recorded in `docs/dev/EDITOR_SMOKE.md`) covering each item below, plus a manual pass by the user before release:
...
- `docs/dev/EDITOR_SMOKE.md` (append: phase 6 run)
...
- `docs/dev/NOW.md`, `docs/dev/PLAN.md` (orchestrator state), `**/*.gd.uid` sidecars
...
- `docs/dev/EDITOR_SMOKE.md` (append: phase 7 run and the render-check command output)
...
- `docs/dev/EDITOR_SMOKE.md` (append: phase 8 runs, the version matrix runs on 4.4 and 4.7)
- `docs/dev/DESIGN.md` (modify: release checklist boxes ticked with the evidence per line)
- `docs/dev/CURRENTNESS_AUDIT.md` (modify: the two deferred notes retired)
...
- Affects: `docs/dev/DESIGN.md`, `docs/dev/PLAN.md`, `docs/dev/SPIKE_NOTES.md`, and the committed screenshots the release checklist requires.
