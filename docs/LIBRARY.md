...
61 functions, one row per manifest under `addons/goshade_turbo/library/`.
...
Regenerate this table after a library change and paste the output over it:
...
| `fieldops/ease` | `ease` | (field) -> field | in: pow(x, x + 1). out: 1 - pow(1 - x, 2 - x). blended by bias. Own construction, handwritten by kidsmeal. At 0.5 blend it mirrors smoothstep. |
...
| `fieldops/ratchet` | `ratchet` | (field) -> field | Own construction, handwritten by kidsmeal. mod(x + 1, x * x) descending sawtooth over a pow(1 - x, x + steep) ease-in floor, blended by x. The reset at 0.99 finishes the last tooth, which mod would only wrap at exactly 1. |
...
| `filter/dither` | `dither` | (color) -> color (filter) | ordered dithering via a per-pixel hash threshold before quantization, standard shader dither construction (threshold from this library's hash() of the integer pixel coordinate FRAGCOORD.xy, so the pattern is per pixel and independent of UV scale). |
...
| `generative/cell_borders` | `cell_borders` | () -> field (generator) | Two-pass exact Euclidean distance to Voronoi edges over a jittered grid, per Inigo Quilez, "Voronoi Edges" (improved/exact version), https://iquilezles.org/articles/voronoilines/: pass one finds the nearest feature point, pass two takes the minimum distance from p to the perpendicular bisector plane between that nearest point and every other feature point within a wider neighborhood. Ported to this project's own generative/hash() in place of the article's hash2(), and to this project's own coordinate and loop-variable naming; the bisector-distance math is unchanged. |
| `generative/cell_borders_edge_warp` | `cell_borders_edge_warp` | () -> field (generator) | Edge distance from generative/cell_borders (Inigo Quilez, "Voronoi Edges"), displaced by a two-channel simplex noise domain warp whose amplitude is 1 - smoothstep(0, edge_band, unwarped edge distance). Gating the warp by the field's own unwarped value, so the displacement is confined to a band around the cracks, is an own construction, handwritten by kidsmeal (GoShade Turbo, 2026-09-16). |
| `generative/cell_borders_round` | `cell_borders_round` | () -> field (generator) | Two-pass exact Euclidean distance to Voronoi edges over a jittered grid, per Inigo Quilez, "Voronoi Edges" (improved/exact version), https://iquilezles.org/articles/voronoilines/, with the second pass's min() replaced by the polynomial smooth minimum from Inigo Quilez, "Smooth minimum", https://iquilezles.org/articles/smin/. The smooth minimum can dip below the true minimum near corners, so the result is clamped at 0. Ported to this project's own generative/hash() in place of the article's hash2(), and to this project's own coordinate and loop-variable naming; the bisector-distance math is unchanged. The substitution of smin for min in pass two, the roundness parameter, and the clamp at 0 are this project's own combination. |
| `generative/cell_borders_round_varied` | `cell_borders_round_varied` | () -> field (generator) | Two-pass exact Euclidean distance to Voronoi edges over a jittered grid, per Inigo Quilez, "Voronoi Edges", https://iquilezles.org/articles/voronoilines/, with the second pass's min() replaced by the polynomial smooth minimum from Inigo Quilez, "Smooth minimum", https://iquilezles.org/articles/smin/. The per-cell radius, drawn from a hash of the owning cell's id so every plate rounds differently, is an own construction, handwritten by kidsmeal (GoShade Turbo, 2026-09-16). |
| `generative/cellular_edges` | `cellular_edges` | () -> field (generator) | F2 - F1 cellular edge construction over a jittered grid, per Inigo Quilez, "Voronoi Edges", https://iquilezles.org/articles/voronoilines/. |
...
| `generative/clock` | `clock` | () -> field | t = fract(TIME * speed); sine 0.5 - 0.5 * cos(TAU * t), triangle abs(2t - 1), sawtooth t. Standard constructions. Ignores position; feeds add or mix to animate a field value. Sawtooth added before a fract is the same as adding unbounded time, since fract drops the whole part. |
| `generative/fbm` | `fbm` | () -> field (generator) | fractional Brownian motion: standard sum-of-octaves construction. See D. Ebert et al., "Texturing & Modeling: A Procedural Approach" (3rd ed., 2003), ch. 2. Octaves sum signed simplex noise (snoise remapped back to -1..1) and the sum is offset by 0.5 so the default settles around 0.5 instead of 0. |
| `generative/hash` | `hash` | () -> field (generator) | Hash without Sine, hash12: fract/dot mixing of the input, no trigonometry, so integer lattice inputs get no mirrored or striped output. David Hoskins, https://www.shadertoy.com/view/4djSRW |
...
| `generative/perlin` | `perlin` | () -> field (generator) | classic 2D gradient (Perlin) noise: Ken Perlin, "An Image Synthesizer", SIGGRAPH 1985. Gradients here come from an angle derived through this library's own hash(), not a permutation table; that substitution is an implementation choice, not copied from any specific source. Quintic fade 6t^5 - 15t^4 + 10t^3 per Perlin, "Improving Noise", SIGGRAPH 2002. Output scaled by sqrt(0.5) then offset 0.5 so the 0..1 range is filled; unit gradients bound the raw value to about +-0.707. |
